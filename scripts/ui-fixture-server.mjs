...
<script>
window.desktop = {
  getAppVersion: async () => '0.22.0-fixture',
...
const fixturePlaylistVideos = Array.from({ length: 4 }, (_, index) => ({
  ...fixtureVideo,
  id: `PLDEMO${String(index + 1).padStart(5, '0')}`,
  title: `播放列表课程 ${index + 1}`,
  playlistTitle: '长列表状态隔离演示',
...
app.get('/api/resolve', (request, response) => {
  const query = String(request.query.url ?? '');
  if (query.includes('playlist-long')) {
    response.json({
      success: true,
      data: {
        kind: 'playlist',
        title: '长列表状态隔离演示',
        videoCount: fixturePlaylistVideos.length,
        videos: fixturePlaylistVideos,
      },
    });
    return;
  }
  if (query.includes('playlist-short')) {
    const videos = fixturePlaylistVideos.slice(0, 2).map((video) => ({
      ...video,
      playlistTitle: '短列表状态隔离演示',
    }));
    response.json({
      success: true,
      data: {
...
app.post('/api/download', (request, response) => {
  ...
  const playlistTaskIds = tasks.map((task) => task?.videoId);
  if (playlistTaskIds.some((videoId) => String(videoId).startsWith('PLDEMO'))) {
    const expected = ['PLDEMO00001', 'PLDEMO00002'];
    if (JSON.stringify(playlistTaskIds) !== JSON.stringify(expected)) {
      response.status(400).json({
        success: false,
        error: {
          code: 'INVALID_PARAM',
          message: `验收失败：短列表提交了错误的视频集合 ${playlistTaskIds.join(', ')}`,
        },
      });
      return;
    }
    response.json({
      success: true,
      data: {
        taskIds: ['fixture-playlist-1', 'fixture-playlist-2'],
        conflicts: [],
        renamed: [],
      },
    });
