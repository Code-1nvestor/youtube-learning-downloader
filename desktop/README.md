...
设置页可以通过受限 IPC 生成脱敏诊断报告。主进程只汇总白名单状态字段和最近日志尾部，
遮蔽 Cookie、认证字段、代理凭据、URL 查询参数与已知本机路径；用户必须在系统“另存为”
窗口确认后才会写入文件，应用不会自动上传报告。

设置页的“浏览…”按钮通过受限 Electron IPC 只打开目录选择器，不向网页开放
Node.js 或任意文件系统访问权限。

下载结果的“打开/文件夹”操作也使用受限 IPC：渲染页面只传任务 UUID，桌面主进程
再从本机后端读取已完成任务的可信输出路径，并在确认文件仍存在后调用 Windows。
网页不能把任意磁盘路径直接交给桌面主进程打开。
